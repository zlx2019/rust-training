...
/// 
/// ```
pub fn gen_password(length: u8, upper: bool, lower: bool, number: bool, symbol: bool) -> anyhow::Result<String>{
    let mut pass_set = Vec::new();
    let mut chars = Vec::new();
    let mut rng = rand::thread_rng();
    // 为了保证能包含指定的类型字符，每种字符需要单独生成一次
    if upper{
        chars.extend_from_slice(UPER_CASE_CHARS);
        pass_set.push(*UPER_CASE_CHARS.choose(&mut rng).unwrap())
    }
    if lower {
    ...
    // 补充剩余位
    let need = length - pass_set.len() as u8;
    for _ in 0..need{
    ...
    // 打乱字符顺序
    pass_set.shuffle(&mut rng);
    let password: String = String::from_utf8(pass_set)?;
    // 校验生成的密码强度
    let password_strength = zxcvbn::zxcvbn(&password, &[]);
    println!("password strength: {}", password_strength.score());
    return Ok(password);
}